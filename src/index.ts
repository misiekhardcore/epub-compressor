...
async function extractTo(epubPath: string, outDir: string): Promise<void> {
...
async function processFiles(
  ...
  for (const filePath of entries) {
    ...
    try {
      ...
      console.warn("Failed to process", rel, (e as Error).message);
      // continue — don't let one failure stop the whole job
    }
  }
}

async function minifyHtmlFile(filePath: string): Promise<void> {
...
async function minifyCssFile(filePath: string): Promise<void> {
  const src = await fsp.readFile(filePath, "utf8");
  const out = csso.minify(src, { comments: false }).css;
  await fsp.writeFile(filePath, out, "utf8");
}

async function minifyJsFile(filePath: string): Promise<void> {
  const src = await fsp.readFile(filePath, "utf8");
...
async function optimizeImage(filePath: string, quality = 80): Promise<void> {
  ...
  if (out.length < input.length) await fsp.writeFile(filePath, out);
}

async function minifySvg(filePath: string): Promise<void> {
  const src = await fsp.readFile(filePath, "utf8");
...
async function walk(dir: string): Promise<string[]> {
  const files: string[] = [];
  async function _walk(p: string): Promise<void> {
